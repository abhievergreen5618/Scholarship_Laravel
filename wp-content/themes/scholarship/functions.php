...
function scholarship_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'scholarship' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'scholarship_custom_background_args',
			array(
...
function scholarship_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'scholarship' ),
			'id'            => 'sidebar-1',
...
function scholarship_scripts() {
	wp_enqueue_style( 'scholarship-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'scholarship-style', 'rtl', 'replace' );

	wp_enqueue_script( 'scholarship-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
