...
<div class="col-md-12">
    <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form id="class-add-form" action="{{isset($data) ? route('admin.subject.update') : route('admin.scholarshiptype.store') }}" method="POST">
            @csrf
            @isset($data)
            ...
            <div class="card-body">
                <div>
                    <div class="form-group mb-2">
                        <label for="name">{{ __('Scholarship Name') }}</label>
                        <input type="text" class="form-control @error('name') {{ 'is-invalid' }} @enderror" id="name" name="name" placeholder="Scholarship Name" value="{{@old('name',$data->name)}}">
                    </div>
                    @error('name')
                    <div>
